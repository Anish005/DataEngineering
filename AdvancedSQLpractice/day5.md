...
with total_transaction_per_customer as (
    select cust_id, sum(total_order_cost) as total_transaction_amount
    from card_orders group by cust_id
),
ranked as (
    select c.id , c.first_name, c.last_name,
        dense_rank() over(order by t.total_transaction_amount desc) as rnk
    from customers c 
    join total_transaction_per_customer t on c.id = t.cust_id
)
