...
select eq.product_name  , ex.product_id ,count(transaction_id) as num_transaction  from 
Excel_sql_transaction_data as ex
join  excel_sql_inventory_data as eq on ex.product_id = eq.product_id
group by ex.product_id , eq.product_name
order by ex.product_id
