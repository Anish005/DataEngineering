...
With employee_cnt as (
	Select cnt(emp_id) as emp_cnt from ms_emp_projects group by project_id
)

select p.ptitle , round(ceil(p.budget/e.emp_cnt)) as each_employee from ms_projects p
join ms_emp_projects  e on p.id = e.project_id order by each_employee desc
